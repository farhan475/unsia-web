                    ...
                    <tr class="hover:bg-gray-50 transition group">
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-3">
                                ...
                                <span class="font-bold text-gray-800 text-sm">{{ $dosen->nama }}</span>
                            </div>
                        </td>
                        <td class="px-6 py-4 text-sm text-gray-600 font-mono">{{ $dosen->nidn }}</td>
                        <td class="px-6 py-4">
                        ...
                        <td class="px-6 py-4 text-sm text-gray-500">{{ $dosen->email }}</td>
                        <td class="px-6 py-4 text-right">
                            <form action="{{ route('admin.dosen.destroy', $dosen->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus dosen ini? Semua data booking terkait akan ikut terhapus.');">
                                @csrf
                                @method('DELETE')
        ...
        <div class="bg-white w-full max-w-lg rounded-xl shadow-2xl overflow-hidden transform transition-all"
            ...
            <form action="{{ route('admin.dosen.store') }}" method="POST" class="p-6 space-y-4">
                ...
                <div>
                    ...
                    <input type="text" name="name" required placeholder="Contoh: Dr. Budi Santoso, M.Kom" 
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="nidn" required placeholder="Nomor Induk" 
                               class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-[#00588a] outline-none">
                    </div>
                    <div>
                ...
                <div>
                    ...
                    <input type="email" name="email" required placeholder="nama@unsia.ac.id" 
                           class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-[#00588a] outline-none">
                </div>
                <div>
                ...
                <div class="flex justify-end gap-3 pt-2">
                    <button type="button" @click="openModal = false" class="px-4 py-2 rounded-lg text-sm font-bold text-gray-500 hover:bg-gray-100 transition">Batal</button>
                    <button type="submit" class="bg-[#00588a] text-white px-6 py-2 rounded-lg text-sm font-bold hover:bg-[#00466e] shadow transition">Simpan</button>
                </div>
            </form>
        </div>
    </div>
