...
<div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
    ...
    <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-100 flex items-center justify-between">
        <div>
            <p class="text-sm font-medium text-gray-500">Taping Hari Ini</p>
            <p class="text-3xl font-bold text-gray-800">{{ $stats['today'] }}</p>
        </div>
        <div class="w-12 h-12 bg-blue-50 text-[#00588a] rounded-lg flex items-center justify-center text-xl">
            <i class="fa-solid fa-video"></i>
        </div>
    </div>
    <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-100 flex items-center justify-between">
        <div>
            <p class="text-sm font-medium text-gray-500">Sedang Diedit</p>
            <p class="text-3xl font-bold text-gray-800">
                {{ \App\Models\Booking::where('status', 'Editing')->count() }}
            </p>
        </div>
    ...
    <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-100 flex items-center justify-between">
        <div>
            ...
            <p class="text-3xl font-bold text-gray-800">{{ \App\Models\Video::count() }}</p>
        </div>
                    ...
                    <tr class="hover:bg-gray-50 transition">
                        ...
                        <td class="px-6 py-4 text-sm">
                            <div class="font-medium text-gray-800">
                                {{ \Carbon\Carbon::parse($item->tanggal_taping)->format('d M Y') }}
                            </div>
                            <div class="text-gray-500 text-xs">{{ $item->sesi }}</div>
                        </td>
                        <td class="px-6 py-4 text-center">
                            <span class="px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wide {{ $item->status_badge_atribut }}">
                                {{ $item->status }}
                            </span>
                        </td>
                        <td class="px-6 py-4 text-right">
                            @if($item->status === 'Pending')
                                <form action="{{ route('admin.workflow.update', $item->id) }}" method="POST" class="inline">
                                    ...
                                    <button class="bg-blue-600 text-white px-3 py-1.5 rounded-lg text-xs font-bold hover:bg-blue-700 shadow-sm transition">
                                        Approve
                                    </button>
                                </form>
                                <form action="{{ route('admin.workflow.update', $item->id) }}" method="POST" class="inline">
                                    ...
                                    <button class="bg-white text-red-600 border border-red-200 px-3 py-1.5 rounded-lg text-xs font-bold hover:bg-red-50 transition">
                                        Reject
                                    </button>
                                </form>
                            ...
                            @elseif($item->status === 'Ready')
                                <div x-data="{ open: false }" class="inline-block relative">
                                    <button
                                        @click="open = true"
                                        class="bg-green-600 text-white px-3 py-1.5 rounded-lg text-xs font-bold hover:bg-green-700 shadow-sm transition flex items-center gap-1"
                                    >
                                            ...
                                            <form action="{{ route('admin.workflow.publish', $item->id) }}" method="POST">
                                                ...
                                                <div class="mb-3">
                                                    ...
                                                    <input
                                                        type="text"
                                                        name="judul_final"
                                                        value="{{ $item->matkul->nama_mk }} - {{ $item->topik }}"
                                                        class="w-full border p-2 rounded text-sm focus:ring-2 focus:ring-green-500"
                                                        required
                                                    >
                                                </div>
                                                <div class="mb-5">
                                                ...
                                                <div class="flex justify-end gap-3">
                                                    ...
                                                    <button
                                                        class="bg-green-600 text-white px-5 py-2 rounded text-sm font-bold hover:bg-green-700"
                                                    >
                                                        Simpan dan Publish
                                                    </button>
                                                </div>
