...
@section('content')
<!-- Filter Bar (Dummy Visual) -->
<div class="flex gap-4 mb-8">
    <select class="border border-gray-300 rounded-lg px-4 py-2 text-sm bg-white focus:ring-2 focus:ring-[#00588a] outline-none">
    ...
    <button class="bg-[#00588a] text-white px-4 py-2 rounded-lg text-sm font-bold hover:bg-[#00466e]">Filter</button>
</div>
...
<div class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-6">
    ...
    <div class="bg-white border border-gray-200 rounded-xl shadow-sm hover:shadow-lg transition-all duration-300 group overflow-hidden flex flex-col">
        ...
        <div class="p-4 flex-1 flex flex-col">
            <h4 class="font-bold text-gray-800 text-sm line-clamp-2 mb-1" title="{{ $video->judul_final }}">
                {{ $video->judul_final }}
            </h4>
            <p class="text-xs text-gray-500 mb-4">{{ $video->booking->user->name }}</p>
            
            <div class="mt-auto border-t border-gray-100 pt-3 flex justify-between items-center">
                <span class="text-[10px] text-gray-400 bg-gray-100 px-2 py-0.5 rounded">{{ $video->semester }}</span>
                <a href="{{ $video->link_video }}" target="_blank" class="text-xs font-bold text-[#00588a] hover:underline flex items-center gap-1">
                    Tonton <i class="fa-solid fa-external-link-alt"></i>
                </a>
            </div>
        </div>
    </div>
